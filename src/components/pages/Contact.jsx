...
import { 
  ...
  FaLinkedinIn, FaTwitter, FaFacebookF, FaArrowRight, FaPaperPlane 
} from "react-icons/fa";
        ...
        <div className="container mx-auto relative z-10">
          <motion.div initial="hidden" animate="visible" variants={fadeInUp} className="text-center lg:text-left">
            ...
            <h1 className="text-6xl md:text-8xl lg:text-9xl font-black tracking-tighter leading-none mb-10 uppercase italic">
              CONTACT <br/>
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-[var(--grms-blue)] to-indigo-600">THE NEXUS.</span>
            </h1>
          </motion.div>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
            ...
            >
              <div className="bg-white/70 backdrop-blur-2xl p-8 md:p-12 rounded-[3.5rem] border border-white shadow-[0_40px_100px_rgba(0,0,0,0.08)] relative overflow-hidden group">
                ...
                <h2 className="text-3xl font-black mb-10 text-slate-950 uppercase tracking-tight italic">Initiate Message</h2>
                
                <form onSubmit={handleSubmit} className="space-y-8 relative z-10">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <InputField label="Full Name" name="name" type="text" value={formData.name} onChange={handleChange} required />
                    <InputField label="Email Address" name="email" type="email" value={formData.email} onChange={handleChange} required />
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <InputField label="Company Name" name="company" type="text" value={formData.company} onChange={handleChange} />
                    <InputField label="Phone Number" name="phone" type="tel" value={formData.phone} onChange={handleChange} />
                  </div>

                  <div className="relative group">
                    <label className="absolute -top-3 left-4 px-2 bg-white text-[10px] font-black text-slate-400 tracking-widest uppercase group-focus-within:text-[var(--grms-blue)] transition-colors">Service Focus</label>
                    <select
                      name="service"
                    ...
                    >
                      ...
                      {["AI Systems", "Data Architecture", "Cloud Infrastructure", "Enterprise Audit"].map((s) => (
                  ...
                  <div className="relative group">
                    <label className="absolute -top-3 left-4 px-2 bg-white text-[10px] font-black text-slate-400 tracking-widest uppercase group-focus-within:text-[var(--grms-blue)] transition-colors">Briefing</label>
                    <textarea
                      name="message"
                  ...
                  >
                    ...
                    <span className="relative flex items-center justify-center gap-4 tracking-widest">
                      TRANSMIT DATA <FaPaperPlane className="group-hover:translate-x-2 group-hover:-translate-y-2 transition-transform" />
                    </span>
                  </button>
                </form>
              </div>
            ...
            <div className="lg:col-span-5 space-y-10">
              <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeInUp}>
                <h2 className="text-4xl font-black mb-8 text-slate-950 italic uppercase tracking-tighter">Support Grid</h2>
                
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-6">
                  <ContactCard icon={<FaEnvelope />} title="Digital Mail" detail="info@gdinexus.com" sub="sales@gdinexus.com" link="mailto:info@gdinexus.com" />
                  <ContactCard icon={<FaPhone />} title="Secure Line" detail="+1 (555) 123-4567" sub="24/7 Global Support" link="tel:+15551234567" />
                  <ContactCard icon={<FaMapMarkerAlt />} title="HQ Terminal" detail="123 Tech Street" sub="Silicon Valley, CA 94027" />
                  <ContactCard icon={<FaClock />} title="Operational Sync" detail="Mon - Fri: 09:00 - 18:00" sub="Weekend Support via Portal" />
                </div>
              </motion.div>
              ...
              <div className="bg-slate-950 p-10 rounded-[3rem] shadow-2xl relative overflow-hidden">
                ...
                <h3 className="text-white text-xl font-black mb-8 uppercase tracking-widest italic">Nexus Social Link</h3>
      ...
      <section className="py-20 px-6">
        <div className="container mx-auto">
          <div className="relative group">
            ...
            <div className="relative h-[500px] bg-slate-200 rounded-[4rem] overflow-hidden shadow-2xl border border-white/50 flex items-center justify-center">
              {/* This is a placeholder for your actual map component */}
              <div className="absolute inset-0 bg-cover bg-center opacity-40 grayscale" style={{backgroundImage: "url('https://images.unsplash.com/photo-1526778548025-fa2f459cd5c1?auto=format&fit=crop&q=80&w=1000')"}}></div>
              <div className="relative z-10 text-center">
                <div className="w-20 h-20 bg-[var(--grms-blue)] rounded-full flex items-center justify-center text-white text-3xl mx-auto mb-4 animate-bounce shadow-2xl">
                  <FaMapMarkerAlt />
                </div>
                <h3 className="text-slate-950 text-2xl font-black uppercase tracking-widest">Nexus HQ Terminal</h3>
                <p className="text-slate-600 font-bold">ACTIVE CONNECTION ESTABLISHED</p>
              </div>
            </div>
          </div>
        </div>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-6">
        ...
        <div>
          <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-300 mb-1 group-hover:text-[var(--grms-blue)] transition-colors">{title}</h4>
          <p className="text-lg font-black text-slate-900 leading-tight mb-1">{detail}</p>
          <p className="text-sm font-medium text-slate-400">{sub}</p>
          {link && (
            <a href={link} className="inline-flex items-center gap-2 text-[var(--grms-blue)] text-xs font-black uppercase mt-3 hover:gap-4 transition-all">
